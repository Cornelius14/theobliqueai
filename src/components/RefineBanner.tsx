...
export default function RefineBanner({ plan, onInsert }: Props) {
  if (!plan || !plan.items.length) return null;

  return (
    <div className="mt-4 rounded-lg border border-amber-400/30 bg-amber-500/10 px-4 py-3">
      <div className="mb-2 font-semibold text-amber-200">We need a quick refinement:</div>
      ...
      <div className="mt-3 text-xs text-amber-300/80">
        Tip: click a chip to insert into your query, then hit <b>Parse</b> again.
      </div>
    </div>
  );
}